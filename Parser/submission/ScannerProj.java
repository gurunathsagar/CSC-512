...
public class ScannerProj{
	...
	public ScannerProj(String fileName) throws FileNotFoundException{

		this.src = new FileInputStream(fileName);
		this.stream = new InputStreamReader(src);
		this.latestToken = new Token(ScannerProj.TokenType.NONE, "");
	...
	/*public static void main(String[] args){
		try{
			ScannerProj sc = new ScannerProj(args[0]); // Scanner object with file name from command line
			Token tk = new Token();
			
			while(sc.hasMoreTokens()){ // Loop will run till more tokens are present.
				tk = sc.getNextToken();
				// Token modified and written if matches condition
				if ( tk.tokenType== TokenType.IDENTIFIER && !( "main".equals(tk.tokenName) )) {
					//sc.destStream.print("cs512" + sc.getNextToken().getTokenName() + " "); 
					System.out.println("token = " + tk.tokenName);
				}
